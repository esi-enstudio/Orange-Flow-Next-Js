...
export default function PermissionsPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    // Sanitize permission name: lowercase, spaces/dashes to underscores, remove special chars
    ...
    <div className="space-y-6 animate-in fade-in duration-500 p-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
        ...
        <div className="flex-1">
          {loading ? (
            <div className="h-full py-20 flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-orange-500"/></div>
          ) : paginatedPermissions.length === 0 ? (
            <div className="h-full py-20 flex items-center justify-center text-gray-500">No permissions found</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 p-4">
              {paginatedPermissions.map((perm) => (
                <div key={perm.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-800/50 rounded-xl group transition-all hover:bg-orange-50 dark:hover:bg-orange-500/5 border border-transparent hover:border-orange-200 dark:hover:border-orange-500/20">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-white dark:bg-slate-900 border dark:border-slate-700 flex items-center justify-center text-gray-400 group-hover:text-orange-500 transition-colors shadow-sm">
                      <Key className="w-4 h-4" />
                    </div>
                    <div className="flex flex-col">
                      <span className="text-sm font-mono font-bold text-gray-700 dark:text-gray-300 group-hover:text-orange-700 dark:group-hover:text-orange-400 transition-colors">{perm.name}</span>
                      {isNew(perm.created_at) && (
                        <span className="text-[8px] font-bold bg-green-500 text-white px-1.5 py-0.5 rounded-full w-fit animate-pulse">NEW</span>
                      )}
                    </div>
                  </div>
                  <button 
        ...
        <div className="p-4 border-t dark:border-slate-800 bg-gray-50/30 dark:bg-slate-900/30 flex items-center justify-between text-[10px] text-gray-500 uppercase tracking-widest font-bold">
          <div>Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredPermissions.length)} of {filteredPermissions.length} permissions</div>
          {totalPages > 1 && <div>Page {currentPage} / {totalPages}</div>}
        </div>
      </div>
