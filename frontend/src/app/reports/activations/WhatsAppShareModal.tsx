...
interface Props {
  ...
  year: number;
  onClose: () => void;
  /** Renders the dashboard exactly as displayed into a PNG blob. */
  generateImage: () => Promise<Blob | null>;
  /** Builds a WhatsApp-friendly text summary of the current report. */
  buildText: () => string;
}
...
type SendFormat = "image" | "text";

export default function ActivationWhatsAppShareModal({ open, houseId, month, year, onClose, generateImage, buildText }: Props) {
  const { t } = useLanguage();
  ...
  const send = async () => {
    if (!houseId || !chatId) {
      toast.error(t("activation_report.whatsapp.no_group_selected"));
      return;
    }
    ...
    try {
      ...
      if (format === "image") {
        ...
        if (!blob) throw new Error(t("activation_report.error_loading"));
        body.image_base64 = await new Promise<string>((resolve, reject) => {
        ...
        if (!text) throw new Error(t("activation_report.whatsapp.no_text"));
        body.text = text;
      }
      await apiClient.post("/reports/activations/whatsapp", body, { headers: houseHeader });
      toast.success(t("activation_report.whatsapp.success"));
      closeModal();
    } catch (e) {
      toast.error((e as Error).message || t("activation_report.whatsapp.error"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const formats: Array<{ key: SendFormat; icon: typeof ImageIcon; label: string }> = [
    { key: "image", icon: ImageIcon, label: t("activation_report.whatsapp.image") },
    { key: "text", icon: AlignLeft, label: t("activation_report.whatsapp.text") },
  ];
  ...
  return (
    <>
      <AnimatePresence>
        {open && (
          <motion.div key="activation-whatsapp-share-overlay"
            initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-gray-100 dark:border-slate-800 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-gray-900 dark:text-gray-100">{t("activation_report.whatsapp.title")}</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400">{t("activation_report.dashboard_title")}</p>
              </div>
              <button
                onClick={closeModal}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                ...
                <div className="flex-1 text-center sm:text-left">
                  <p className={cn("font-semibold flex items-center justify-center sm:justify-start gap-2",
                    ...
                    {checking && !status ? (
                      ...
                      <><CheckCircle2 className="w-4 h-4" /> {t("activation_report.whatsapp.connected")} ({status.state})</>
                    ) : (
                      <><AlertCircle className="w-4 h-4" /> {t("activation_report.whatsapp.disconnected")}</>
                    )}
                  </p>
                    ...
                    {status?.qr
                      ...
                      : !status?.connected && status?.state === "not_configured"
                        ...
                        : !status?.connected && !status?.qr
                          ? t("activation_report.whatsapp.no_status")
              ...
              <div>
                <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 block">
                  {t("activation_report.whatsapp.group")}
                </label>
                {groups.length > 0 ? (
                  ...
                  <p className="text-sm text-gray-400 bg-gray-50 dark:bg-slate-800/40 rounded-xl border border-dashed border-gray-200 dark:border-slate-700 px-3 py-3">
                    {t("activation_report.whatsapp.no_groups")}{!status?.connected ? ` — ${t("activation_report.whatsapp.disconnected")}` : ""}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 block">
                  {t("activation_report.whatsapp.format")}
                </label>
              ...
              {format === "image" && (
                <div>
                  <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 block">
                    {t("activation_report.whatsapp.caption")}
                  </label>
                  <input
                    type="text"
                    value={caption}
                    onChange={(e) => setCaption(e.target.value)}
                    placeholder={t("activation_report.whatsapp.caption_placeholder")}
            ...
            <div className="px-6 py-4 border-t border-gray-100 dark:border-slate-800 flex items-center justify-end gap-2">
              ...
              <button
                onClick={send}
                disabled={loading || !status?.connected || !chatId}
              ...
              >
                ...
                {loading ? t("activation_report.whatsapp.sending") : t("activation_report.whatsapp.send")}
              </button>
            </div>
          </motion.div>
        </motion.div>
