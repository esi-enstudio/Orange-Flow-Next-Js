...
import ActivationWhatsAppShareModal from "./WhatsAppShareModal";
...
export default function ActivationDashboardPage() {
  ...
  const buildReportText = (): string => {
    ...
    const lines: string[] = [];
    lines.push(`*${t("activation_report.dashboard_title")} • ${getMonthName(month)} ${year}*`);
    lines.push(`${house ? `${house.name}` : t("activation_report.all_houses")}`);
    lines.push("");
    lines.push(`🎯 ${t("activation_report.monthly_target")}: *${formatNumber(s.monthly_target)}*`);
    lines.push(`✅ ${t("activation_report.achievement")}: *${formatNumber(s.achievement)}* (${s.achievement_percentage}%)`);
    if (s.yesterday_activation) {
      lines.push(`📅 ${t("activation_report.yesterday_activation", { count: s.yesterday_activation })}`);
    }
    lines.push(`⏳ ${t("activation_report.remaining")}: ${formatNumber(s.remaining)} (${t("activation_report.days_remaining")}: ${s.days_remaining})`);
    lines.push(`⚡ ${t("activation_report.daily_required")}: ${s.daily_required}`);
    lines.push(`📈 ${t("activation_report.projection")}: *${formatNumber(Math.round(s.projection))}*`);

    const top = data.top_performers;
    if (top) {
      const sections: Array<{ label: string; rows: EmployeePerformance[] }> = [
        { label: t("activation_report.rso_performance"), rows: top.rso },
        { label: t("activation_report.bp_performance"), rows: top.bp },
        { label: t("activation_report.cc_performance"), rows: top.cc },
      ];
      for (const sec of sections) {
        if (!sec.rows?.length) continue;
        lines.push("");
        lines.push(`🏆 ${sec.label}:`);
        sec.rows.slice(0, 3).forEach((emp, i) => {
          lines.push(`${i + 1}. ${emp.name} — ${formatNumber(emp.achievement)}/${formatNumber(emp.target)} (${emp.percentage}%)`);
        });
      }
    }
    return lines.join("\n");
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
            ...
            {showShareMenu && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 z-50 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg shadow-lg py-1 min-w-[160px]">
                  <button
                    onClick={() => {
                      if (hasPermission("reports.whatsapp_share")) {
                        setShowShareMenu(false);
                        setShowWhatsAppShare(true);
                      } else {
                        handleShareImage("whatsapp");
                      }
                    }}
      ...
      {hasPermission("reports.whatsapp_share") && showWhatsAppShare && (
        <ActivationWhatsAppShareModal
          open
          houseId={selectedHouseId ? Number(selectedHouseId) : null}
          month={month}
          year={year}
          onClose={() => setShowWhatsAppShare(false)}
          generateImage={generateReportImage}
          buildText={buildReportText}
        />
      )}
