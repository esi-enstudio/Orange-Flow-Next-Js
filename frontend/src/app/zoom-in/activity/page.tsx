...
export default function ActivitiesPage() {
  ...
  const fetchActivities = async () => {
    setLoading(true);
    try {
      const res = await apiClient.get("zoom-in/activities", {
        params: { include_inactive: true },
      });
      setActivities(res.data);
    } catch {
      toast.error(t("zoom_in.activities.messages.load_failed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (a: Activity) => {
    setEditing(a);
    setForm({ name: a.name, name_bn: a.name_bn || "" });
    setFormErrors({});
    setFormOpen(true);
  };
  ...
  const validate = () => {
    ...
    if (!form.name.trim()) errs.name = t("zoom_in.activities.validation.name_required");
    setFormErrors(errs);
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        await apiClient.put(`zoom-in/activities/${editing.id}`, {
          ...payload,
          is_active: editing.is_active,
        });
        toast.success(t("zoom_in.activities.messages.update_success"));
      } else {
        await apiClient.post("zoom-in/activities", payload);
        toast.success(t("zoom_in.activities.messages.create_success"));
      }
      closeForm();
      fetchActivities();
    } catch (err: any) {
      if (err.response?.status === 409) {
        setFormErrors({ name: t("zoom_in.activities.validation.name_exists") });
      } else {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await apiClient.delete(`zoom-in/activities/${deleting.id}`);
      toast.success(t("zoom_in.activities.messages.delete_success"));
      closeDelete();
      fetchActivities();
    } catch {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t("zoom_in.activities.title")}</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t("zoom_in.activities.description")}</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-gray-300 dark:border-slate-800 p-16 text-center">
          ...
          <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">{t("zoom_in.activities.messages.no_data")}</h3>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-900/50">
                  <th className="text-left px-6 py-4 font-bold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">{t("zoom_in.activities.table.name")}</th>
                  <th className="text-left px-6 py-4 font-bold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">{t("zoom_in.activities.table.name_bn")}</th>
                  <th className="text-left px-6 py-4 font-bold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">{t("zoom_in.activities.table.status")}</th>
                  <th className="text-right px-6 py-4 font-bold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">{t("zoom_in.activities.table.actions")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50 dark:divide-slate-800/50">
                {filtered.map((a) => (
                  <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4 text-gray-900 dark:text-gray-100 font-medium">{a.name}</td>
                    <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{a.name_bn || "—"}</td>
                    <td className="px-6 py-4">
                      <span className={cn(
                        ...
                        {a.is_active ? t("zoom_in.activities.status.active") : t("zoom_in.activities.status.inactive")}
                      </span>
                    </td>
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-slate-800">
              <div>
                <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">
                  {editing ? t("zoom_in.activities.edit") : t("zoom_in.activities.create")}
                </h2>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{t("zoom_in.activities.description")}</p>
              </div>
              <button
                onClick={closeForm}
            ...
            <form onSubmit={handleFormSubmit} className="p-6 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">
                    {t("zoom_in.activities.fields.name")} <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    value={form.name}
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">
                    {t("zoom_in.activities.fields.name_bn")}
                  </label>
                  <input
                    type="text"
                    value={form.name_bn}
              ...
              {editing && (
                <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                  ...
                  />
                  <span className="font-bold">{t("zoom_in.activities.fields.is_active")}</span>
                </label>
              )}
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-slate-800">
                ...
                >
                  ...
                  {editing ? t("common.save_changes") : t("zoom_in.activities.create")}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl animate-in zoom-in-95 duration-200 overflow-hidden">
            <div className="p-8 flex flex-col items-center text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">
                {t("zoom_in.activities.messages.delete_confirm")}
              </h3>
