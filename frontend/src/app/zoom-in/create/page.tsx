...
export default function CreateZoomInEventPage() {
  ...
  useEffect(() => {
    ...
    apiClient.get("zoom-in/allocations/check", {
      params: { house_id: houseId, event_type_id: eventTypeId, thana, month },
    }).then(res => {
      setAllocationInfo({
      ...
      setAllocationInfo({
        has_allocation: false, allocated_count: 0, created_count: 0, remaining: 0,
        loading: false, error: t("zoom_in.messages.load_failed"),
      });
    });
  }, [houseId, date, eventTypeId, thana]);
