"""Active SSO Report image builder for WhatsApp/Telegram delivery.

Renders a summary card with RSO-level SIM activation performance as a PNG image.
"""
import io
import math
...
from app.services.active_sso_report_service import ActiveSsoReportService
...
def _render_image(house_name: str, house_code: str, dashboard: dict) -> bytes:
    ...
    content_lines = []
    content_lines.append(("title", f"Active SSO Report ({month_year})"))
...
async def build_active_sso_report_image(db: AsyncSession, house_id: int) -> bytes:
    """Build the Active SSO report as a PNG image."""
    today = now_naive().date()
    ...
    svc = ActiveSsoReportService(db, house_id, start_date, end_date)
