...
async def _load_report_data(db: AsyncSession, house_id: int, today: date):
    ...
    service = ActivationReportService(db, house_id, today.month, today.year, target_role="HOUSE")
    summary = await service.get_summary()

    # Team targets are computed inside GaLiveQueryBuilder.build_all() from the
    # supervisor_targets / rso_targets / bp_targets tables, scoped to ACTIVE
    # employees only. Carry them through from the service summary so the report
    # stays consistent (active-only) across supervisor/RSO/BP rows.
    summary["supervisor_target"] = data["summary"].get("supervisor_target", 0)
    summary["supervisor_target_map"] = data["summary"].get("supervisor_target_map", {}) or {}
    summary["rso_target"] = data["summary"].get("rso_target", 0)
    summary["bp_target"] = data["summary"].get("bp_target", 0)
